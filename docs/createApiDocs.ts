...
import { createDetails } from "docusaurus-plugin-openapi-docs/src/markdown/createDetails";
import { createDetailsSummary } from "docusaurus-plugin-openapi-docs/src/markdown/createDetailsSummary";
...
export function createApiPageMD2({
    title,
    api: {
      deprecated,
    ...
    return render([
      ...
      createPermissionsTab(extensions),
  ...
  function createPermissionsTab(extensions) {
    if (!extensions || typeof extensions !== "object") {
        return undefined;
    }

    const permissionsObject = extensions.find(item => item.key === 'x-permissions');
    const permissions = permissionsObject ? permissionsObject.value : [];
    ...
    return createDetails({
        ...
        open: true,
        style: {},
        children: [createDetailsSummary({
            className:
              "openapi-markdown__details-summary-response",
            children: [
              create("strong", { children: `Permissions` }),
            ],
          }),
          create("div", {
          ...
          create("ul", {
            ...
            children: createNodes(permissions),
          })
        ],
    });
}
...
function createNodes(
    ...
    permissions.forEach((element) => {
        nodes.push(create("SchemaItem", {
            collapsible: false,
            name: element,
            required: false,
            schemaName: null,
            qualifierMessage: null,
            schema: null,
          }));
    });
